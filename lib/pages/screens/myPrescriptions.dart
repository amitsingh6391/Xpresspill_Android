...
class _MyPrescriptionsState extends State<MyPrescriptions> {
  ...
  _getCurrentUserId()async
  {
    ...
    print(currentUserId);
    var prescriptions=prescriptionsRef.where("lockedBy",isEqualTo: currentUserId).snapshots();
    setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        centerTitle: true,
        actions: [
          IconButton(icon: Icon(AntDesign.logout),
            onPressed: (){
              _userService.logOut(context);
            },)
        ],
      ),
      body: _allPrescriptions==null?Center(child: CircularProgressIndicator()):buildPrescriptionResults(),
